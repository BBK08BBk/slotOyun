...
public class IzgaraServisi
{
    ...
    public void SetGetPayTableBase(Func<float[]> fn) => _getPayTableBase = fn;
    ...
    public void RenderSpritesOnlyForCells(IEnumerable<Vector2Int> hucreler, int[,] grid)
    {
        if (_hucreler == null || grid == null || _sembolSpriteListesi == null) return;
        foreach (var p in hucreler)
        {
            ...
            img.type = Image.Type.Simple;
            img.preserveAspect = true;
            if (sym >= 0 && sym < _sembolSpriteListesi.Count)
                img.sprite = _sembolSpriteListesi[sym];
            Color c = img.color;
            c.a = 1f;
            img.color = c;
    ...
    public int RandomNonScatterSymbol(int[,] countGrid, int maxPerSymbol = -1)
    {
        if (_sembolSpriteListesi == null || _sembolSpriteListesi.Count <= 1) return 0;
    ...
    public int RandomSymbolWithScatterChance(int[,] countGrid, bool bonusAktif, int maxPerSymbol = -1)
    {
