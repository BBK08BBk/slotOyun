...
public class SenaryoYoneticisi : MonoBehaviour
{
    ...
    [Tooltip("2→3: Spin eşiği (300 spin, 5 bahis değişimi, 1 kez bakiye %20 düşüş)")]
    ...
    /// <summary>Aşama 2'de bakiye aşama girişine göre en az %20 düştü mü (1 kez).</summary>
    private bool _asama2BakiyeYuzde20Dustu;
    ...
    void Start()
    {
        ...
        if (hosgeldinizText != null)
            hosgeldinizText.text = "Hoşgeldiniz " + (GameManager.I?.ActivePlayer?.playerName ?? "Oyuncu");
        SetupUI();
    ...
    {
        if (this != I && (gelistirmeModu || !senaryoAktif)) return;
        ...
        switch (mevcutAsama)
        {
            ...
            case SenaryoAsama.Asama2_KontrolBende:
                // 2→3: Bu aşamanın çıkış şartlarından en az 2 tanesi sağlanmışsa VEYA spinFarki ≥ 100 ise geç
                gecildi = CikisSartlariniDegerlendir(2, out _, out _) >= 2 || spinFarki >= 100;
    ...
    private int CikisSartlariniDegerlendir(int asamaNo, out List<string> tamamlanan, out List<string> kalan)
    {
        ...
        switch (asamaNo)
        {
            ...
            case 2:
                ...
                if (_asama2BakiyeYuzde20Dustu) tamamlanan.Add("1 kez bakiye %20 düşüş"); else kalan.Add("1 kez bakiye %20 düşüş");
                break;
            case 3:
    ...
    public void AsamaGecir(SenaryoAsama yeni)
    {
        ...
        if (yeni == SenaryoAsama.Asama2_KontrolBende)
            _asama2BakiyeYuzde20Dustu = false;
    ...
    public void SpinTamamlandi(int kazanc, int bahis)
    {
        ...
        if (mevcutAsama == SenaryoAsama.Asama2_KontrolBende && _asamaGirisBakiyesi > 0 && mevcutBakiye <= _asamaGirisBakiyesi * 80 / 100)
            _asama2BakiyeYuzde20Dustu = true;
    ...
    void GecisSartGuncelle()
    {
        if (gecisSartText != null)
        {
            ...
            else
            {
                ...
                gecisSartText.text = (tam.Count > 0 ? "Tamamlanan: " + string.Join("\n", tam) + "\n" : "") + (kal.Count > 0 ? "Kalan: " + string.Join("\n", kal) : "Kalan: —");
            }
        }
        SartMetinleriniGuncelle();
    }
